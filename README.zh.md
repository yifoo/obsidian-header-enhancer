...
- 全局与单文档双级控制
- 自动标题级别检测，可配置分隔符（`.`、`-`、`/`、`,`）
- 默认从 H2 开始编号，保留 H1 作为文档标题
- 中文起始标题可使用中文数字编号（`一、二、三、`），下级标题使用数字编号（`1.`、`1.1`）
- 灵活的起始级别（H1-H6）、中文标题编号开关和 YAML 前置元数据覆盖
- 带确认对话框的批量操作
...
2. **开始使用**
   ...
   - 开启中文标题编号后，中文起始标题会生成 `## 一、标题`，下一级标题会生成 `### 1.子标题`
...
**编号配置**
- **起始级别**：从哪个标题级别开始编号（默认 H2，保留 H1 作为文档标题）
- **结束级别**：在哪个标题级别停止编号
- **自动检测**：开启后自动识别文档中的标题级别，并遵循设置中的起始级别
- **分隔符**：编号间的分隔符，支持 `.`（1.1）、`-`（1-1）、`/`（1/1）、`,`（1,1）。起始级别数字编号后也会添加分隔符，并直接连接标题文字，如 `1.标题`。
- **起始数字**：第一个标题的编号（默认为 1）
- **中文标题编号**：开启后，中文起始标题使用 `一、二、三、`，下一级标题使用直接连接标题文字的 `1.小节`，更深层级保持 `1.1 条目` 等数字层级编号。
...
- 中文文档：开启中文标题编号，生成 `一、章节`、`1.小节`、`1.1 条目`
...
- 提升整体稳定性和用户体验
- 默认起始编号级别改为 H2
- 新增中文标题编号设置（中文起始标题使用 `一、二、三、`）
- 起始级别数字编号后会添加数字分隔符，并直接连接标题文字，如 `1.标题`
- 构建产物输出到 `dist/`
